...
def create_loan(loan: schemas.LoanCreate, db: Session = Depends(get_db)):
    try:
        return Loans_crud.create_loan(db=db, loan=loan)
    except Exception as e:
        return JSONResponse(status_code=500,content=f'erroe due to {e}')
...
def return_book(id,db:Session=Depends(get_db)):
    try:
        db_loan = Loans_crud.return_book(db, loan_id=id)
        if db_loan is None:
...
def check_overdue_loans(db: Session = Depends(get_db)):
    try:
        updated_loans = Loans_crud.check_and_update_overdue_loans(db)
...
def delete_loan(loan_id: int, db: Session = Depends(get_db)):
    try:
        db_loan = Loans_crud.delete_loan(db, loan_id=loan_id)
        if db_loan is None:
